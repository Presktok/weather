...
function ExpandableSection({
  ...
  open,
  onToggle,
  className = 'border-slate-700 bg-ocean-800',
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${className}`}>
      ...
      >
        ...
        <span
          className={`shrink-0 text-slate-400 text-xs font-medium px-2 py-1 rounded bg-slate-800/80 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
          aria-hidden
        >
          ▼
        </span>
      </button>
      {open && <div className="px-4 pb-4 border-t border-slate-700/40">{children}</div>}
    </div>
  )
}
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-end gap-2 text-xs">
        <button
          type="button"
          onClick={expandAll}
          className="px-3 py-1.5 rounded-lg bg-slate-700/80 text-slate-300 hover:bg-slate-600 transition"
        >
          Expand all
        </button>
        <button
      ...
      {section('recommendation', {
        title: `Recommended ${recommendation.route_label}`,
        subtitle: recommendation.route_name,
        className: 'border-green-500/40 bg-green-500/10',
        titleClassName: 'font-bold text-green-400 text-lg',
        children: (
          <>
            {charterAlert && (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className={`rounded-lg p-3 border ${recIsB ? 'border-slate-700' : 'border-green-500/50 bg-green-500/5'}`}>
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  cost {scores.components.route_a.cost} + delay {scores.components.route_a.delay} + risk{' '}
                  {scores.components.route_a.risk} + laycan {scores.components.route_a.laycan}
                </p>
              </div>
              <div className={`rounded-lg p-3 border ${recIsB ? 'border-green-500/50 bg-green-500/5' : 'border-slate-700'}`}>
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  cost {scores.components.route_b.cost} + delay {scores.components.route_b.delay} + risk{' '}
                  {scores.components.route_b.risk} + laycan {scores.components.route_b.laycan}
                </p>
              </div>
            </div>
            <p className="text-xs text-slate-500 mt-3">
              Weights: Cost {scores.weights.cost * 100}% · Delay {scores.weights.delay * 100}% · Risk{' '}
              {scores.weights.risk * 100}% · Laycan {scores.weights.laycan * 100}%
            </p>
                  ...
                  {breakdown.components.map((c) => (
                    <li key={c.dimension} className="text-sm">
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            c.points >= 0 && !c.penalty_applied_each
                              ? 'text-green-400 font-medium'
                              : 'text-red-400 font-medium'
                          }
                        >
                          {c.penalty_applied_each
                            ? `-${Math.abs(c.points)} pts each`
              ...
              {[['Route A', laycanCtx?.route_a], ['Route B', laycanCtx?.route_b]].map(([name, lc]) => (
                <div key={name} className="rounded-lg bg-ocean-900/80 p-3 border border-slate-700">
                  ...
                  <p className="text-white text-sm">
                    ETA: {lc?.predicted_eta_display} ({lc?.voyage_days}d voyage)
                  </p>
      ...
      {weatherImpact &&
        section('weather-impact', {
          title: 'Weather Impact Summary (Route A)',
          subtitle: weatherImpact.baseline_note,
          children: (
            <>
              ...
              <MetricRow
                ...
                highlight={weatherImpact.high_risk_leg_count > 0}
              />
      ...
      {section('high-risk', {
        title: 'High Risk Zones (Route A)',
        subtitle: `${riskZones.length} zone${riskZones.length === 1 ? '' : 's'}`,
        className: 'border-red-500/30 bg-red-500/5',
        titleClassName: 'font-semibold text-red-400',
        children:
          riskZones.length === 0 ? (
                ...
                <li key={z.waypoint} className="rounded-lg bg-ocean-900/80 p-3 border border-red-500/20">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-slate-300">
                    {z.wave_height_m != null && (
                      <span>
                        Wave: <strong className="text-slate-100">{z.wave_height_m} m</strong>
                      </span>
                    )}
                    {z.wind_speed_kn != null && (
                      <span>
                        Wind: <strong className="text-slate-100">{z.wind_speed_kn} kn</strong>
                      </span>
                    )}
                    <span>
                      Speed loss: <strong className="text-slate-100">{z.expected_speed_loss_kn} kn</strong>
                    </span>
                    <span>
                      Delay: <strong className="text-slate-100">{z.expected_delay_hours} hrs</strong>
                    </span>
                    <span className="col-span-2">
                      Risk: <RiskBadge risk={z.risk_level} />
                    </span>
                  </div>
                </li>
              ))}
      ...
      {section('savings', {
        title: 'Route Comparison vs Baseline',
        className: 'border-blue-500/30 bg-blue-500/5',
        titleClassName: 'font-semibold text-blue-300',
        children: (
          <div className="grid grid-cols-1 gap-3 text-sm">
            <div className="rounded-lg bg-ocean-900 p-3">
              ...
              <p className="text-slate-300 mt-1">
                Fuel: <span className="text-white font-semibold">{savings.without_optimization.fuel_mt} MT</span>
              </p>
              <p className="text-slate-300">
                Cost:{' '}
                <span className="text-white font-semibold">
                  ${savings.without_optimization.fuel_cost_usd.toLocaleString()}
                </span>
              </p>
              <p className="text-slate-300">
                Delay: <span className="text-white font-semibold">{savings.without_optimization.delay_hours} hrs</span>
              </p>
            </div>
            <div className="rounded-lg bg-ocean-900 p-3 border border-green-500/30">
              ...
              <p className="text-slate-300 mt-1">
                Fuel: <span className="text-white font-semibold">{savings.with_recommended.fuel_mt} MT</span>
              </p>
              <p className="text-slate-300">
                Cost:{' '}
                <span className="text-white font-semibold">
                  ${savings.with_recommended.fuel_cost_usd.toLocaleString()}
                </span>
              </p>
              <p className="text-slate-300">
                Delay: <span className="text-white font-semibold">{savings.with_recommended.delay_hours} hrs</span>
              </p>
            </div>
            <div
              className={`rounded-lg p-3 border ${net.fuel_cost_is_savings ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}
            >
              <p
                className={`text-xs uppercase font-semibold ${net.fuel_cost_is_savings ? 'text-green-400' : 'text-red-400'}`}
              >
      ...
      {section('waypoints', {
        title: 'Waypoint Detail (Route A)',
        subtitle: `${routeA.analysis.legs.length} legs`,
        children: (
          <div className="overflow-x-auto max-h-80 overflow-y-auto -mx-1">
